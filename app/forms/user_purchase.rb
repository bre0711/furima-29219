class UserPurchase
  ...
  validates :postal_number, format: { with: /\A\d{3}[-]\d{4}\z/, message: 'ハイフン有りの7桁の数字でで入力してください' }
  validates :phone_number, format: { with: /\A\d{1,11}\z/, message: 'ハイフン無しの11桁以内で入力してください' }
